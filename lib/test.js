    ...
    var fieldsById = d3.nest().key(function(d) {return d.id;})
            ...
            radios1.onclick = function(){
                if (field.id == "PECons" && radios1.checked){
                    ...
                    colors = mycolor_orange.map(function(rgb) { return d3.hsl(rgb); });
                    d3.select("#map-container > svg > text").remove();
                    d3.select("#map-container > svg").append("text").attr("x",0).attr("y","93.3%").attr("dy",".35em").text("Units: GDP PPP Per Capita (Thousand 2005 US Dollar)");
   ...
   // var start = Date.now();
    ...
    // update the data
         countries.data(features)
                .select("title")
                .text(function(d) {
                    return [d.properties.country, fmt(value(d))==0 ? "NA" : parseFloat(Math.round(fmt(value(d))*100)/100).toFixed(2)].join(": ");
                });
